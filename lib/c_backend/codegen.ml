...
let create ?(indent="    ") ~prog ~ptr_size () =
...
and codegen_expression (env: t) (expr: Expr.t) =
  ...
  | NewI64 value -> (
    ...
    | Ir.Ptr64 ->
      ps env Primitives.Value.mk_i64;
      ps env "(";
      ps env value;
      ps env ")"
    | Ir.Ptr32 ->
  ...
  | I32Binary(op, left, right) -> (
    let name = Primitives.Bin.prim_i32 op in
    ps env name;
    ps env "(";
...
let codegen_program ?indent ~prog ~includes ~init_calls ~ptr_size (declarations: Typedtree.Declaration.t list) =
  let env = create ?indent ~prog ~ptr_size () in
