...
let unexpected_error (env : Lex_env.t) (loc : Loc.t) value =
  lex_error env loc (Lex_error.Unexpected (quote_token_value value))

let unexpected_error_w_suggest (env : Lex_env.t) (loc : Loc.t) value suggest =
  lex_error env loc (Lex_error.UnexpectedTokenWithSuggestion (value, suggest))

let illegal (env : Lex_env.t) (loc : Loc.t) =
  lex_error env loc (Lex_error.Unexpected "token ILLEGAL")
...
let decode_identifier =
  let assert_valid_unicode_in_identifier env loc code =
    ...
    | any
    | eof ->
      lex_error env loc Lex_error.IllegalUnicodeEscape
...
type result =
  ...
  | Comment of Lex_env.t * Loc.t Comment.t
...
let token (env : Lex_env.t) lexbuf : result =
  ...
  | "internal" -> Token (env, T_INTERNAL)
  | "module" -> Token (env, T_MODULE)
  ...
  | eof ->
    let env =
      if is_in_comment_syntax env then
        let loc = loc_of_lexbuf env lexbuf in
        lex_error env loc Lex_error.UnexpectedEOS
      else
        env
    in
    Token (env, T_EOF)
...
let rec regexp_body env buf lexbuf =
  ...
  | eof ->
    let loc = loc_of_lexbuf env lexbuf in
    let env = lex_error env loc Lex_error.UnterminatedRegExp in
    (env, "")
  | ('\\', line_terminator_sequence) ->
    let loc = loc_of_lexbuf env lexbuf in
    let env = lex_error env loc Lex_error.UnterminatedRegExp in
