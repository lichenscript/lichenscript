...
module Bindings (F : Cstubs.FOREIGN) = struct
  ...
  let expr_block = F.foreign "BinaryenBlock" F.(module_ @-> string @-> ptr expression @-> returning expression)
  ...
  let add_function = F.foreign "BinaryenAddFunction"
    F.(module_ @-> string @-> binary_type @-> binary_type @-> ptr binary_type @-> expression @-> returning function_)
